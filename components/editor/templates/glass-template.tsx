...
export function GlassTemplate({ profile, links, socials, font }: TemplateProps) {
  const activeSocials = socials?.filter(s => s.isActive === true) || [];
  const activeLinks = links?.filter(l => l.isActive === true) || [];
        ...
        <AnimatePresence mode="popLayout">
          {activeLinks.map((link) => {
            ...
            <motion.a
              ...
              className="block w-full text-center text-sm font-medium transition-all duration-200 overflow-hidden group relative flex items-center justify-center bg-white/10 border border-white/20 backdrop-blur-md rounded-2xl hover:bg-white/20 hover:scale-[1.02] p-3.5"
            >
              {link.image ? (
                <div className="flex items-center w-full h-full">
                  <div className="w-14 h-14 overflow-hidden flex-shrink-0 bg-black/20 rounded-2xl">
